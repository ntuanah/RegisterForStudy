...
const TheoryClassCard = ({ theoryClass, index }) => {
      ...
      <div className="border border-[#0A4174] p-4 rounded-xl bg-white shadow-sm">
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-5 items-end">
            ...
            <button className="text-white font-medium border border-[#0A4174] rounded-full p-2 bg-[#5483B3] hover:bg-gray-200 hover:text-[#5483B3] cursor-pointer transition-all duration-300 hover:-translate-y-1">
        ...
        <div className="flex mt-4 pt-3 justify-between items-center">
          <div>
            <p className="text-sm font-medium text-[#5483B3] bg-blue-50 px-3 py-1.5 rounded-lg border border-slate-200">
              <span className="text-[#0A4174] font-bold">Lịch:</span> {theoryClass.schedules?.[0]?.roomName || "Chưa xếp phòng"}, {theoryClass.schedules?.[0]?.dayOfWeekName || ""}, Tiết {theoryClass.schedules?.[0]?.startPeriod || ""}-{theoryClass.schedules?.[0]?.endPeriod || ""}
            </p>
          </div>
          <div className="flex gap-3">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" strokeWidth="1.5" d="M12 2v3m-6.995 6.995h-3m2.926-7.063l2.12 2.122m12.022-2.129L16.95 7.047m-9.9 9.9l-2.12 2.12M11 11l10 4.4l-4.437 1.163L15.4 21z"/></svg>
              Chọn phòng giờ học
            </button>
            ...
            {theoryClass.children?.length > 0 && (
              ...
              >
                {openPracticeClassCard ? "Ẩn thực hành" : `Xem ${theoryClass.children.length} lớp TH`}
              </button>
            )}
          </div>
        </div>
      ...
      {openPracticeClassCard && theoryClass.children && (
        <div className="ml-8 pl-6 py-2 space-y-4 relative">
          ...
          {theoryClass.children.map((practiceClass, idx) => (
            <PracticeClassCard key={practiceClass.id} practiceClass={practiceClass} index={idx + 1} />
          ))}
        </div>
      )}

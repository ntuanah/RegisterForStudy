...
const CohortTable = () => {
    const [openEditCohort, setOpenEditCohort] = useState(false);
  return (
    <div className="border border-slate-200 rounded-xl shadow-sm mt-5">
            ...
            <td className="px-6 py-4">
              <div className="flex gap-4 text-[#5483B3]">
                <svg
                  ...
                  onClick={() => setOpenEditCohort(true)}
                >
                  <path
                    fill="none"
                    stroke="currentColor"
      ...
      {openEditCohort && <EditCohort close={() => setOpenEditCohort(false)} />}
    </div>
  );
};

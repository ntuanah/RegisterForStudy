...
import { deleteSubjectFromDefaultSectionAPI, getTemplateTreeAPI } from "../../../service/programService";
import { toast } from "react-toastify";
import AddSubjectProgramDefault from "../Modal/AddSubjectProgramDefault";

const ProgramDefaultTable = () => {
  ...
  const fetchTemplateTree = async () => {
    try {
      setIsLoading(true);
      const response = await getTemplateTreeAPI();
      const { data } = response;

      if (data.code === 1000) {
        setTreeData(data.result);
      } else {
        toast.error(data.message || "Lỗi lấy cấu trúc chương trình mẫu!");
      }
    } catch (error) {
      toast.error("Không thể tải cấu trúc chương trình mẫu!");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteSubject = async (sectionId, subjectId) => {
    const isConfirm = window.confirm(
      "Bạn có chắc chắn muốn xóa môn học này khỏi chương trình mẫu?",
    );
    if (!isConfirm) return;

    try {
      const response = await deleteSubjectFromDefaultSectionAPI(
        sectionId,
        subjectId,
      );
      const { data } = response;
  ...
  if (isLoading) {
    return (
      <div className="border border-slate-200 rounded-xl shadow-sm p-5 text-center text-slate-500 mt-5">
        Đang tải chi tiết chương trình đào tạo mẫu...
      </div>
    );
  }
  ...
  if (!treeData || !treeData.groups || treeData.groups.length === 0) {
    return (
      <div className="border border-slate-200 rounded-xl shadow-sm p-5 text-center text-slate-500 mt-5">
        Chưa có dữ liệu cho chương trình mẫu.
      </div>
    );
  }
  ...
  return (
    <div className="border border-slate-200 rounded-xl shadow-sm mt-5 overflow-hidden overflow-x-auto">
      <table className="w-full min-w-[800px] text-left border-collapse">
        <thead>
          <tr className="bg-blue-50">
            ...
            <th className="px-6 py-2 text-[10px] font-bold text-slate-400 w-15 text-center whitespace-nowrap">
              Thao tác
            </th>
          </tr>
        </thead>
      ...
      {selectedSectionId && (
        <AddSubjectProgramDefault
          sectionId={selectedSectionId}
          close={() => setSelectedSectionId(null)}
          refresh={fetchTemplateTree}
        />
      )}
    </div>
  );
...
export default ProgramDefaultTable;

...
export const downloadLecturerExcelTemplateAPI = async () => {
  return await axiosClient.get("/auths/excel-template-lecturer", {
    responseType: "blob",
  });
};

export const importLecturerExcelAPI = async (file) => {
  const formData = new FormData();
  formData.append("file", file);

  return await axiosClient.post("/auths/import-lecturers-excel", formData, {
    headers: {
      "Content-Type": "multipart/form-data",
    },
  });
